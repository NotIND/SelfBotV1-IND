...
def selfbot_menu(bot):
    ...
    print(f"""\n{Fore.RESET}
                           ...
                           ╚█║████╔╝   ██║   ██║╚██████╔╝       ██║   ╚██████╔╝╚██████╔╝███████╗
                            ╚╝╚═══╝    ╚═╝   ╚═╝ ╚═════╝        ╚═╝    ╚═════╝  ╚═════╝ ╚══════╝\n""".replace('█', f'{b}█{y}'))
    print(f"""{y}------------------------------------------------------------------------------------------------------------------------
{w}raadev {b}|{w} https://github.com/NotIND/SelfBotV1-IND/ {b}|{w} https://github.com/NotIND/SelfBotV1-IND/ {b}|{w} https://github.com/NotIND/SelfBotV1-IND/ {b}|{w} 
https://github.com
{y}------------------------------------------------------------------------------------------------------------------------\n""")
...
async def on_ready():
    if platform.system() == "Windows":
        ctypes.windll.kernel32.SetConsoleTitleW(f"SelfBot v{__version__} - Made By IND")
        os.system('cls')
    else:
        os.system('clear')
    selfbot_menu(bot)
...
async def help(ctx):
    ...
    help_text = f"""
**IND SelfBot | Prefix: `{prefix}`**\n
**Commands:**\n
> :space_invader: `{prefix}ind` - Show my social networks.
...
@bot.command(aliases=['ind'])
async def ind(ctx):
    await ctx.message.delete()

    embed = f"""**MY SOCIAL NETWORKS | Prefix: `{prefix}`**\n
    > :pager: `YouTube `\n*https://youtube.com/@itsnotind*
    > :computer: `GitHub Page`\n*https://github.com/NotIND*
...
async def spam(ctx, amount: int=1, *, message_to_send: str="https://discord.gg/PKR7nM9j9U"):
    ...
    try:
        if amount <= 0 or amount > 9000:
            await ctx.send("> **[**ERROR**]**: Amount must be between 1 and 9000", delete_after=5)
            return
